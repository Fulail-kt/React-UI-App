...
export const updateUser = async (req, res) => {
  ...
  try {
    if(roles.length<1){
      return res.status(400).json({sucess:false,message:'Roles cannot be Empty'})
    }
    const updatedUser = await User.findByIdAndUpdate(id, { firstName, lastName, email, roles }, { new: true }).populate({path:'roles',model:'Role'});
    console.log(updateUser,"upd")
